...
class RBP():
    ...
    def __init__(self, protein_name, ):
        pdb_chain = protein_name.split('_')
        if len(pdb_chain)==2:
            self.pdb_id, self.chain=pdb_chain[0], pdb_chain[1]  # complex, protein, rna
        elif len(pdb_chain) == 1:

            self.pdb_id, self.chain=protein_name.split('_')[0], ''
        ...
        self.seq, self.index2resid, self.resid2Residue, self.all_res_id=self._get_pdb_seq(self.chain, kd_scale)
        ...
        for chain in self.model.child_list:
            for Residue in chain.child_list:
                if Residue.get_resname() in scale.keys():
                    ...
                    all_res_id.append(Residue.id[1])
                    index+=1
        return res_seq, index2resid, resid2structure, all_res_id
        ...
        with open(out_file) as pid:
            for line in pid.readlines():
                if line[2] == '#':
                    flag_1=True
                    continue
                if flag_1 and (self.chain=='' or line[11] == self.chain[-1]):
                    flag_2=True
                    res_name=line[13]
    ...
    def _call_psiblast(self):
        ...
        args=[psiblast_bin, "-db", uniprot_database, "-query", fasta_dir, "-evalue", '0.001', "-num_iterations", '3',
              "-out_ascii_pssm", pssm_dir]
    ...
    def _get_pKa_feature(self):
        ...
        for item in pKa_scale.keys():
            pKa_mapping[item]=(
            (pKa_scale[item][0] - 1.82) / (2.38 - 1.82), (pKa_scale[item][1] - 8.80) / (10.96 - 8.80),
    ...
    def _get_Physicochemical_feature(self):
        ...
        for item in li_scale.keys():
            physio_scale[seq3(item).upper()]=(
            (li_scale[item][0] - (-1)) / (1 - (-1)), ((li_scale[item][1] - 2) / (4 - 2)))
        return physio_scale
    ...
    def get_data(self):
        ...
        for index, res_id in enumerate(self.surface_res):
            try:
                ...
                continue
        batch_dict={'data': np.array(data), 'surface_res_id': res_surface, 'all_res_id':self.all_res_id}
